...
def main():
    ...
    if scale_flg == "Interactive":
        ...
        with st.expander("Configure Execution Settings", expanded=True):
            # Paths for execution
            af_input_path = st.text_input("AF Input Path", value=os.path.expanduser("~/af_input"),
                                          help="Path to AlphaFold input directory.")
            af_output_path = st.text_input("AF Output Path", value=os.path.expanduser("~/af_output"),
            ...
            logger.debug(
                f"Execution settings: af_input_path={af_input_path}, af_output_path={af_output_path}, model_parameters_dir={model_parameters_dir}, databases_dir={databases_dir}")
    ...
    if scale_flg == "Automated Workflow":
        ...
        if af3_version == "custom":
            af3_container = st.text_input("AlphaFold 3 container",value="~/my_alphafold3_container.sif",
                                             help="Docker image reference or path to AlphaFold 3 singularity container.")
        ...
        if executor == "slurm":
            slurm_account = st.text_input("SLURM account name",value="account",
                                             help="Slurm account name. Run the following command to display your SLURM account name: sacctmgr show user $USER")
            af3_data_pipeline_partition = st.text_input("SLURM Partition for MSA generation",value="cpu_partition",
            ...
            af3_inference_gpu_resources = st.text_input("GPU resources",value="gpu:a100:1",
        ...
        profile_config_yaml = yaml.dump(profile_data, default_flow_style=False)
        ...
        if st.button("Run AlphaFold 3 Snakemake workflow Now ▶️"):
            ...
            with st.spinner('AlphaFold 3 is running...'):
                output_placeholder = st.empty()
                result_code = run_snakemake(snakemake_command, job_dir, placeholder=output_placeholder)
                # Handle success or failure
                if result_code == 0:
                    st.success("AlphaFold 3 Snakemake workflow executed successfully!")
                    # Provide download option
                    st.markdown("### Download Results 📥")
                    zip_data = compress_output_folder(OUTPUT_DIR, OUTPUT_DIR)
                    st.download_button(
                        label="Download ZIP",
                        data=zip_data,
                        file_name=f"af3_workflow_output.zip",
                        mime="application/zip"
                    )
                    logger.info("User downloaded the results ZIP file.")
                else:
                    st.error("An error occurred during the workflow execution.")
